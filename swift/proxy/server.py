...
class OverrideConf(object):
    ...
    :param scope: a string describing the scope of the overridden properties.
    :param conf: the proxy-server config dict.
    :param override_conf: a dict of overriding configuration options.
    """
    def __init__(self, scope, conf, override_conf):
        self.scope = scope
        self.conf = conf
        self.override_conf = override_conf

        self.sorting_method = self._get('sorting_method', 'shuffle').lower()
        self._read_affinity = read_affinity = self._get('read_affinity', '')
        try:
            self.read_affinity_sort_key = affinity_key_function(read_affinity)
        except ValueError as err:
            # make the message a little more useful
            raise ValueError("Invalid read_affinity value for %s: %r (%s)" %
                             (self.scope, read_affinity, err.message))

        write_affinity = self._get('write_affinity', '')
        try:
            self.write_affinity_is_local_fn \
                = affinity_locality_predicate(write_affinity)
        except ValueError as err:
            # make the message a little more useful
            raise ValueError("Invalid write_affinity value for %s: %r (%s)" %
                             (self.scope, write_affinity, err.message))
        value = self._get('write_affinity_node_count',
        ...
        if len(value) == 1:
            ...
            raise ValueError(
                'Invalid write_affinity_node_count value for %s: %r' %
...
class Application(object):
    ...
    def _load_per_policy_config(self, conf):
        ...
        for index, override_conf in conf.get('policy_config', {}).items():
            try:
                ...
                raise ValueError(
                    'Override config must refer to policy index: %s' % index)
            policy = POLICIES.get_by_index(index)
            if policy:
                scope = 'policy %s (%s)' % (policy.idx, policy.name)
                override_confs[policy] = OverrideConf(scope,
                                                      conf,
                                                      override_conf)
                self.logger.debug("Loaded override config for %s" % scope)
            else:
                self.logger.warning(
                    "No policy found for override config, index: %s" % index)
        return override_confs

    def get_conf(self, policy):
        """
        Return policy specific configuration.
    ...
    def check_config(self):
        ...
        for policy, conf in self._override_confs.items():
            if conf._read_affinity and conf.sorting_method != 'affinity':
                self.logger.warning(
                    _("sorting_method is set to '%(method)s', not 'affinity'; "
                      "%(scope)s read_affinity setting will have no effect."),
                    {'scope': conf.scope, 'method': conf.sorting_method})
    ...
    def sort_nodes(self, nodes, policy=None):
        ...
        shuffle(nodes)
        policy_conf = self.get_conf(policy)
...
def parse_per_policy_config(conf):
    ...
    for section, options in all_conf.items():
        ...
        if not policy_ref:
            raise ValueError("Invalid policy config section: %s" % section)
